...
def _infer_doc_type(pdf_name: str) -> DocType:
    ...
    Priority order (checked first wins):
        ...
        2. Commissioned / Completed / Comm keywords -> TBCB_Comm_Report
        3. UC / Under-Construction keywords -> TBCB_UC_Report
    ...
    # 2. Commissioned / Completed
    if re.search(
        r"commission|completed|_comm[_.\s]|^comm_|report_comm|_comm$",
        name_lower,
    ):
    ...
    if re.search(
        r"_uc[_.\s]|^uc_|_uc$|report_uc|tbcb.*uc|uc.*tbcb",
        name_lower,
    ):
