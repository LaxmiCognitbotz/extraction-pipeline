...
def scan_page(page, user_input: str) -> list[dict]:
    ...
    for ol in ol_elements:
        li_elements = ol.query_selector_all(":scope > li")
        for li in li_elements:
            title = li.inner_text().strip().split("\n")[0]
            if not title:
                continue
            if not title_matches(title, user_input):
                continue

            links = extract_child_links(li)
            if links:
                results.append(
                    {
                        "sr_no": len(results) + 1,
...
def make_folder_name(user_input: str, max_len: int = 60) -> str:
    ...
    place_cap = re.findall(r"([A-Z][a-zA-Z\s]+?)\s*\(Phase[^)]*?(\d+(?:\.\d+)?GW)\)", text, re.IGNORECASE)
...
def paginate_all(page):
    # PFCCL uses a single page list; nothing to paginate in current design.
    return
...
def main():
    ...
    ap.add_argument("--query", "-q", default=None, help="Substring of the tender title to match (exact, no breakdown)")
    ap.add_argument("--output", "-o", default="./uploads/PFCCL-INDIA-TENDER", help="Master output directory  (default: ./uploads/PFCCL-INDIA-TENDER)")
    args = ap.parse_args()
